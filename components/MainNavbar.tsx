...
const MainNavbar = () => {
  ...
  return (
    <Navbar position="sticky" maxWidth="full" isBordered>
      <NavbarBrand className="w-fit inline-flex gap-3 items-center divide-x-1 divide-current">
        <Link
          href="/"
          className="text-current text-xl font-bold inline-flex gap-3 items-center justify-start w-fit leading-none select-none"
        >
          <LamdamIcon width={20} height={30} />
          <span>Lamdam</span>
        </Link>
        <div className="text-current text-xs font-normal pl-3">
          by Neuversity
        </div>
      </NavbarBrand>
        ...
        {user.role === "superuser" && (
          <NavbarItem>
            <Dropdown placement="bottom-end">
              ...
              >
                {(item: any) => (
                  <DropdownItem key={item.path}>{item.title}</DropdownItem>
                )}
              </DropdownMenu>
            </Dropdown>
          </NavbarItem>
